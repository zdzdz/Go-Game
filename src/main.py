...
def main():
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and pygame.mouse.get_pressed()[0]:
                ...
                # If pass button is clicekd
                    ...
                    interface.pos_x <= pos[0] <= interface.pos_x + 140
                    and interface.b_pass_pos_y <= pos[1] <= interface.b_pass_pos_y + 40
                    ...
                    interface.pos_x <= pos[0] <= interface.pos_x + 140
                    and interface.w_pass_pos_y <= pos[1] <= interface.w_pass_pos_y + 40
                    and board.pass_count < 2
                ...
                # If ready button is clicked (black)
                    ...
                    interface.pos_x <= pos[0] <= interface.pos_x + interface.ready_width
                    and interface.b_ready_pos_y <= pos[1] <= interface.b_ready_pos_y + interface.butt_height
                    and board.pass_count == 2
                ):
                ...
                # If ready button is clicked (white)
                    ...
                    and board.game_end == True
                ):
                    main()
                # If exit game button is clicked
                elif (
                    interface.pos_x <= pos[0] <= interface.pos_x + interface.quit_width
                    and interface.quit_pos_y <= pos[1] <= interface.quit_pos_y + interface.butt_height
                ):
                    pygame.QUIT
                    sys.exit()
        ...
        if board.game_end == True:
            interface.draw_players_text(WIN, board.captued_white_stones, board.captued_black_stones)
            interface.draw_total_score(
                WIN,
                board.white_territory_count,
                board.captued_black_stones,
                board.black_territory_count,
                board.captued_white_stones,
                board.komi,
            )
